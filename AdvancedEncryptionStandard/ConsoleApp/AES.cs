...
namespace AesLib
{
    public class Aes  // Advanced Encryption Standard
    {
        ...
        private int nBlockSize { get; set; }         // block size in 32-bit words.  Always 4 for AES.  (128 bits).
        private int nKeySize { get; set; }         // key size in 32-bit words.  4, 6, 8.  (128, 192, 256 bits).
        private int nNumberRounds { get; set; }         // number of rounds. 10, 12, 14.

        private byte[] key { get; set; }     // the seed key. size will be 4 * keySize from ctor.
        private byte[,] Sbox { get; set; }   // Substitution box
        private byte[,] iSbox { get; set; }  // inverse Substitution box
        private byte[,] keySchedule { get; set; }      // key schedule array.
        private byte[,] Rcon { get; set; }   // Round constants.
        private byte[,] State { get; set; }  // State matrix

        public Aes(KeySize keySize, byte[] keyBytes)
        {
            SetNbNkNr(keySize);

            this.key = new byte[this.nKeySize * 4];  // 16, 24, 32 bytes
        ...
        {
            this.State = new byte[4, nBlockSize];  // always [4,4]
            for (int i = 0; i < (4 * nBlockSize); ++i)
            {
            ...
            for (int round = 1; round <= (nNumberRounds - 1); ++round)  // main round loop
            {
                SubBytes();
                ShiftRows();
                MixColumns();
        ...
        public void InvCipher(byte[] input, byte[] output)  // decipher 16-bit input
        {
            ...
            for (int round = nNumberRounds - 1; round >= 1; --round)  // main round loop
            {
                InvShiftRows();
                InvSubBytes();
                AddRoundKey(round);
        ...
        {
            this.nBlockSize = 4;     // block size always = 4 words = 16 bytes = 128 bits for AES

            if (keySize == KeySize.Bits128)
            {
                this.nKeySize = 4;   // key size = 4 words = 16 bytes = 128 bits
                this.nNumberRounds = 10;  // rounds for algorithm = 10
            }
            else if (keySize == KeySize.Bits192)
            {
                this.nKeySize = 6;   // 6 words = 24 bytes = 192 bits
                this.nNumberRounds = 12;
            }
            else if (keySize == KeySize.Bits256)
            {
                this.nKeySize = 8;   // 8 words = 32 bytes = 256 bits
                this.nNumberRounds = 14;
            }
        }  // SetNbNkNr()
    ...
    /*f*/  {0x17, 0x2b, 0x04, 0x7e, 0xba, 0x77, 0xd6, 0x26, 0xe1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0c, 0x7d} };
        ...
        private void ShiftRows()
        {
            ...
            for (int r = 1; r < 4; ++r)  // shift temp into State
            {
                for (int c = 0; c < 4; ++c)
                {
                    this.State[r, c] = temp[r, (c + r) % nBlockSize];
                }
            }
        }  // ShiftRows()
        ...
        private void InvShiftRows()
        {
            ...
            for (int r = 1; r < 4; ++r)  // shift temp into State
            {
                for (int c = 0; c < 4; ++c)
                {
                    this.State[r, (c + r) % nBlockSize] = temp[r, c];
                }
            }
        }  // InvShiftRows()
        ...
        {
            this.keySchedule = new byte[nBlockSize * (nNumberRounds + 1), 4];  // 4 columns of bytes corresponds to a word

            for (int row = 0; row < nKeySize; ++row)
            {
                this.keySchedule[row, 0] = this.key[4 * row];
            ...
            for (int row = nKeySize; row < nBlockSize * (nNumberRounds + 1); ++row)
            {
                ...
                if (row % nKeySize == 0)
                {
                    ...
                    temp[0] = (byte)((int)temp[0] ^ (int)this.Rcon[row / nKeySize, 0]);
                    temp[1] = (byte)((int)temp[1] ^ (int)this.Rcon[row / nKeySize, 1]);
                    temp[2] = (byte)((int)temp[2] ^ (int)this.Rcon[row / nKeySize, 2]);
                    temp[3] = (byte)((int)temp[3] ^ (int)this.Rcon[row / nKeySize, 3]);
                }
                else if (nKeySize > 6 && (row % nKeySize == 4))
                {
                    temp = SubWord(temp);
                }

                this.keySchedule[row, 0] = (byte)((int)this.keySchedule[row - nKeySize, 0] ^ (int)temp[0]);
                this.keySchedule[row, 1] = (byte)((int)this.keySchedule[row - nKeySize, 1] ^ (int)temp[1]);
                this.keySchedule[row, 2] = (byte)((int)this.keySchedule[row - nKeySize, 2] ^ (int)temp[2]);
                this.keySchedule[row, 3] = (byte)((int)this.keySchedule[row - nKeySize, 3] ^ (int)temp[3]);
        ...
        public void Dump()
        {
            Console.WriteLine("Nb = " + nBlockSize + " Nk = " + nKeySize + " Nr = " + nNumberRounds);
            Console.WriteLine("\nThe key is \n" + DumpKey());
            Console.WriteLine("\nThe Sbox is \n" + DumpTwoByTwo(Sbox));
